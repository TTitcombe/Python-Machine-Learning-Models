...
class GMM(object):
    ...
    def __init__(self,X,K, a_seed = None):
        ...
        self.K = K
        self.resp = None
        self.params = self.initialise(a_seed)

    def initialise(self, a_seed=None):
        ...
        for i in range(1, self.K + 1):
            params['means_{}'.format(i)] = np.random.rand(2)
            params['covars_{}'.format(i)] = np.random.rand(2,2) + np.eye(2)
    ...
    def findParams(self, n_it):
        for i  in range(n_it):
            self.resp = self.expectation(); # compute responsibilities, i.e., every \\gamma(z_{n,k})
            self.params = self.maximisation() # update the values for the parameters
        return params
